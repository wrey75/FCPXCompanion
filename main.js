...
async function createWindow() {
    ...
    ipcMain.handle('work-status', (event, title) => {
        return new Promise((resolve, reject) => {
            ...
            console.log("send resfresh at "+ new Date());
            resolve(status);
        });
    });

    ipcMain.on('delete-render', (event, arg) => {
        deleteEventDirectory(index, "Render Files");
        return false;
    });
    
    ipcMain.on('delete-transcoded', (event, index) => {
...
function addScanDirectory(path) {
    ...
    nbDirectories++;
    scanDirectory(path).then((newList) => {
        scannedDirectories++;
        refresh();
        newList.forEach(p => addScanDirectory(p));
    });
}
